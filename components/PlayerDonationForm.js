...
export default function PlayerDonationForm({ player, onSubmit }) {
  ...
  const stats = [
    { key: 'points', title: 'Points Scored', description: 'Pledge per point scored by Marcus', avgPerGame: 18.4, icon: '🏀' },
    { key: 'rebounds', title: 'Rebounds', description: 'Pledge per rebound by Marcus', avgPerGame: 4.2, icon: '🏀' },
    { key: 'assists', title: 'Assists', description: 'Pledge per assist by Marcus', avgPerGame: 3.1, icon: '🏀' },
    { key: 'steals', title: 'Steals', description: 'Pledge per steal by Marcus', avgPerGame: 2.8, icon: '🏀' },
    { key: 'blocks', title: 'Blocks', description: 'Pledge per block by Marcus', avgPerGame: 0.5, icon: '🏀' },
    { key: 'threePointers', title: 'Three-Pointers', description: 'Pledge per three-pointer made', avgPerGame: 2.1, icon: '🏀' }
  ];
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-8 w-full max-w-3xl mx-auto text-center relative z-10">
      <h2 className="text-2xl font-bold mb-6">Support {player.name}'s Season</h2>
      ...
      <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
        ...
        <div className="flex justify-center mb-6">
          <button 
            ...
            className={`px-6 py-2 rounded-l-lg font-semibold transition-colors ${
              donationType === 'one-time' ? 'bg-[#EF6C2F] text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            }`}
          >
            One-Time Donation
          </button>
          <button 
            ...
            className={`px-6 py-2 rounded-r-lg font-semibold transition-colors ${
              donationType === 'per-stat' ? 'bg-[#EF6C2F] text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            }`}
          >
            Per-Stat Pledge
          </button>
        ...
        <div className="flex items-center justify-center gap-4 mb-4">
          ...
          <label htmlFor="fundToggle" className="relative inline-flex items-center cursor-pointer">
            <input 
              id="fundToggle" 
          ...
          <span className="text-gray-700 font-semibold">Direct to {player.name.split(' ')[0]}</span>
        </div>
        <p className="text-sm text-gray-600 mb-6">
          Your donation will be directed specifically to {player.name.split(' ')[0]}'s equipment and training needs.
        </p>
              ...
              {[25, 50, 100, 250].map((amount) => (
                <button
                  ...
                  className={`px-4 py-3 rounded-lg font-semibold hover:cursor-pointer transition-colors ${
                    selectedAmount === amount && !customAmount ? 'bg-[#EF6C2F] text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                  }`}
                >
                  ${amount}
                </button>
                ...
                {stats.map((stat) => (
                  <div key={stat.key} className="border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="text-gray-500">per {stat.key === 'threePointers' ? '3PT' : stat.key.slice(0, -1)}</span>
                    </div>
                    <p className="text-xs text-gray-500 mt-2">📊 Avg: {stat.avgPerGame} per game</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-[#2C3E50] text-white p-6 rounded-lg mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-white/10 p-4 rounded-lg text-center">
                  ...
                  <p className="text-xs opacity-80">Based on average performance</p>
                </div>
                <div className="bg-white/10 p-4 rounded-lg text-center">
                  <h4 className="text-sm font-medium mb-1">Monthly Estimate</h4>
                  <p className="text-2xl font-bold">${calculatePledgeEstimate() * 4}</p>
                  <p className="text-xs opacity-80">Based on 4 games per month</p>
                </div>
                <div className="bg-white/10 p-4 rounded-lg text-center">
                  <h4 className="text-sm font-medium mb-1">Season Total</h4>
                  <p className="text-2xl font-bold">${Math.min(calculatePledgeEstimate() * 20, maxCap)}</p>
                  <p className="text-xs opacity-80">Based on 20 game season (capped)</p>
                </div>
              </div>
        ...
        <div className="text-left mb-6">
          <h3 className="text-lg font-bold mb-3">Allocate your donation (optional):</h3>
        ...
        <div className="text-left mb-6">
          ...
          <textarea 
            {...register('message')}
            placeholder={`Enter a message of support for ${player.name.split(' ')[0]}...`}
            rows="4" 
        ...
        <div className="mt-6 p-4 bg-gray-50 rounded-lg text-left">
          ...
          <div className="space-y-1 text-sm text-gray-700">
            <p><span className="font-medium">Type:</span> {donationType === 'one-time' ? 'One-Time Donation' : 'Per-Stat Pledge'}</p>
            <p><span className="font-medium">Amount:</span> ${customAmount || selectedAmount}</p>
            {donationType === 'per-stat' && (
              <>
                <p><span className="font-medium">Estimated per game:</span> ${calculatePledgeEstimate()}</p>
                <p><span className="font-medium">Season cap:</span> ${maxCap}</p>
              </>
            )}
            <p><span className="font-medium">Direction:</span> {fundDirection ? `Direct to ${player.name.split(' ')[0]}` : 'Team Fund'}</p>
            <p><span className="font-medium">Allocation:</span> {selectedAllocation.charAt(0).toUpperCase() + selectedAllocation.slice(1)}</p>
          </div>
        </div>
      </form>
    </div>
